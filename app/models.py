...
class User(db.Model, UserMixin):      # baza danych przechowyje dane każdego z urztkowników, hasło będzie zakodowane
    ...
    review = db.relationship('Vege_Review', backref='author', lazy=True)
...
class Review(db.Model):
    ...
    def __repr__(self):
        return f"Meat_Review:('{self.post_date}')"


class Vege_Review(db.Model):
    id=db.Column(db.Integer, primary_key=True)
    post_date=db.Column(db.Date, default=datetime.utcnow())
    content=db.Column(db.Text, nullable=False)
    user_id=db.Column(db.Integer,db.ForeignKey('user.id'), nullable=False)
    ...
    def __repr__(self):
        return f"Vege_Review:('{self.post_date}')"